...
class ServiceHolidaysList(Document):
	def validate(self):
		self.validate_days()
...
def get_events(start, end, filters=None):
	...
	if start:
		filters.append(['Service Holidays', 'holiday_date', '>', getdate(start)])
	if end:
		filters.append(['Service Holidays', 'holiday_date', '<', getdate(end)])

	return frappe.get_list('Service Holidays List',
		fields=['name', '`tabService Holidays`.holiday_date', '`tabService Holidays`.description', '`tabService Holidays List`.color'],
		filters = filters,
		update={"allDay": 1})